function [ssc] = SSCH(g, slot)
%SSC_FULL 15 slots of SSC,
    if nargin<2
        slot = (0:14);
    end
    ...
    assert(g>=0 && g <= 63, "Group of ssc must in [0, 63]")
    GG = G();
    this_group = GG(g+1, :);
    ...
    if is_full
        ...
        for i=slot+1
            idx = (1:256) + (i-1) * 2560;
            ssc(idx) = code.secondary_sync_code(this_group(i));
        end
    else
        ssc = zeros(2560, 1);
        idx = (1:256);
        ssc(idx) = code.secondary_sync_code(this_group(slot));
    end
end

function [group_vs_slot] = G()
    group_vs_slot = [1 1 2 8 9 10 15 8 10 16 2 7 15 7 16        ; ...
